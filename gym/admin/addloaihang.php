...
if (isset($_POST["addcategory"])) {
    ...
    if (empty($categoryname) || empty($mota)) {
        echo "Error: All fields are required.";
    } else {
        $flag = $region->addcategory($conn, $categoryname,$mota);
        if (!$flag) {
...
if (isset($_GET["action"]) && ($_GET["action"] == "delete")) {
    $flag = $region->deletecategory($conn, $_GET["categoryID"]);
    if (!$flag) {
        echo "Error: insert incorrect!!!";
    }
    header("location:addloaihang.php");
}


if (isset($_GET["action"]) && ($_GET["action"] == "update")) {
    $regionupdate = $region->findcategorybyID($conn, $_GET["categoryID"]);
    if (isset($_POST['updatecategory'])) {
    $region->updateCategory($conn, $_POST['categoryName'], $_POST['mota'], $_GET['categoryID']);
    header("location:addloaihang.php");
}
}
?>
            ...
            <div id="layoutSidenav_content">
                ...
                <form method="POST">
                    <table border="0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>Tên sản phẩm</td>
                                <td><input type="text" name="categoryName" value='<?php
                                    if (isset($_GET["action"]) && ($_GET["action"] == "update")) {
                                        echo $regionupdate["categoryName"];
                                    }
                                    ?>' /></td>
                            </tr>
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    <table border="1">
                        <thead>
                            <tr>
                                <th width="10%">Mã loại sản phẩm</th>

                                <th width="10%">Tên loại Sản phẩm</th>
                                <th width="40%">Mô tả loại sản phẩm</th>
                                <th width="10%">chỉnh</th>
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="?action=update&categoryID=<?php echo $row["categoryID"]; ?>"><img src="../assetss/img/app/icons8-update-50.png" width="30px" /></a>
                                    </td>
                                    <td>
                                        <a href="?action=delete&categoryID=<?php echo $row["categoryID"]; ?>"><img src="../assetss/img/app/icons8-delete-30.png" width="30px"/></a>
                                    </td>
                                </tr>
                                <?php
                            }
