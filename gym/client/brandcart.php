...
if (isset($_GET['action']) && $_GET['action'] == "add") {
    $id = intval($_GET['id']);
    $categoryID = isset($_GET['brandID']) ? $_GET['brandID'] : ""; // Get the categoryID parameter
    ...
    header("Location: brandcart.php?brandID=$categoryID");
    exit();
}
?><html lang="en">
    <head>
        ...
        <div class="product-section mt-150 mb-150">
            <div class="container">
                ...
                if (isset($_GET['brandID'])) {
                    ...
                    $result = $product->showAllbrand1($conn, $brandID);
                    if (mysqli_num_rows($result) > 0) {
                        $counter = 0;
                        //  đếm số sản phẩm trong mỗi hàng
                        while ($row = mysqli_fetch_assoc($result)) {
                            if ($counter % 3 == 0) {
                            ...
                            <div class="col-lg-4 col-md-6 text-center strawberry">
                                <div class="single-product-item">
                                    ...
                                    <a class="cart-btn" href="brandcart.php?action=add&id=<?php echo $row['ProductID']; ?>&brandID=<?php echo $brandID; ?>">Add to cart</a>
                                </div>
                            </div>
                            <?php
                            $counter++;
